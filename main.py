...
def show_tracks_on_no_playlist(conn):
    ...
    rows = queries.get_tracks_on_no_playlist(conn)
    if not rows:
...
def show_most_added_track(conn):
    ...
    row = queries.get_most_added_track(conn)
    if row is None:
...
def show_playlist_durations(conn):
    ...
    rows = queries.get_playlist_durations(conn)
    if not rows:
...
def open_or_build_database():
    ...
    if os.path.exists(DB_PATH):
        ...
        print(f"--- First-run: Building and seeding {DB_PATH} ---")
